...
# |                      FastLanes                       |
# ────────────────────────────────────────────────────────
# data/generated/subintsplit/generate.py
# ────────────────────────────────────────────────────────
#!/usr/bin/env python3
...
Usage
-----
    python3 generate.py                             # regenerate the committed
                                                    # datasets (65536 rows each)
    python3 generate.py --out-dir /tmp/sis-big \\
                        --rows 1048576              # big benchmark datasets,
                                                    # written outside the repo

Without --out-dir the script writes in place, which is how the committed
datasets are produced.  With --out-dir it writes only the three *benchmark*
datasets to DIR/<name>/, using the same per-dataset layout, and never touches
anything inside data/.  Large datasets (~40 MB at 1048576 rows) are deliberately
not committed, so generate them into a scratch directory and point the benchmark
at it.

../encodings/subintsplit_i64/ is a committed test fixture: it is fixed at 65536
rows because test/src/expression_tests/subintsplit_test.cpp depends on that
size.  Regenerating it at a different --rows is therefore refused unless --force
is passed; use --out-dir for large runs instead.
...
def main() -> None:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        help="write only the three benchmark datasets to DIR/<name>/ instead of in-repo; "
        "use this for large datasets, which are not committed",
    )
    ap.add_argument(
        "--force",
        action="store_true",
        help=f"allow regenerating the committed correctness fixture at rows != {DEFAULT_ROWS}",
    )
    ...
    # The correctness fixture is a committed test dependency, so it is only
    # written for an in-repo run and only at its committed size.
    write_correctness = args.out_dir is None
    if write_correctness and args.rows != DEFAULT_ROWS and not args.force:
        raise SystemExit(
            f"refusing to regenerate the correctness fixture at --rows {args.rows}:\n"
            f"  {ENCODINGS_DIR / 'subintsplit_i64'} is committed at {DEFAULT_ROWS} rows and\n"
            "  test/src/expression_tests/subintsplit_test.cpp depends on exactly that size.\n"
            f"  For a large benchmark run use:  --out-dir DIR --rows {args.rows}\n"
            "  (writes the three benchmark datasets outside the repo, fixture untouched)\n"
            "  Pass --force only if you really mean to change the committed fixture."
        )
    ...
    rows = args.rows
    out_dir = args.out_dir if args.out_dir is not None else HERE
